...
sellerRouter.post('/register',expressAsyncHandler(async(req,res)=>{
    ...
    const seller=new Seller({name:req.body.name,
...
sellerRouter.post('/addProduct',expressAsyncHandler(async(req,res)=>{
    ...
    const seller=await Seller.findOne({email:req.body.email});
    const {productList}=seller;
    if(productList && productList.find(x=>x.name===req.body.name)){
        const prod= productList.find(x=>x.name===req.body.name)
        ...
        {
            $set:{"productList.$.countInStock":updatedStock}
        });
       ...
       const sellerUpdate= await Seller.updateOne(
           { "email":req.body.email },
           {$push:{"productList":{name:req.body.name,brand:req.body.brand,
            image:req.body.image,
            category:req.body.category,
            description:req.body.description,
            price:req.body.price,
